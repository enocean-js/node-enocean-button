...
// for node version < 8.0 run with node --harmony-async-await example1.js on
...
enocean.on("ready", function(){               // when ready
	...
	if(process.argv[2] == "on"){
		...
		button.A1.click().then(enocean.close)               // if you pass the word "on" via the command line, invoke the click Event on A1
	}else{
		button.A0.click().then(enocean.close)            // if you pass the word "off" via the command line, invoke the click Event on A0
	}
});
